...
namespace dsa {
	...
	// 构造函数
		...
		private:
			ListNodePosi<T> p;
		public:
			iterator(ListNodePosi<T> _p = nullptr) : p(_p) {}
			T& operator*() {
				T result = T();
				return (p != nullptr) ? p->data : result;
			}
			iterator& operator++() { p = p->succ; return *this; }
			iterator operator++(int) { iterator old = *this; p = p->succ; return old; }
			iterator& operator--() { p = p->pred; return *this; }
			iterator operator--(int) { iterator old = *this; p = p->pred; return old; }
			bool operator==(iterator it) { return p == it.p; }
			bool operator!=(iterator it) { return p != it.p; }
			T* operator->()const { return &(p->data); }
		};
		iterator begin() const { return iterator(first()); } //迭代器，返回首节点位置
		iterator end() const { return iterator(trailer); } //迭代器，返回末节点位置
